 ...
 *
 * Return:  the address of the new node, or NULL if it fai
 */
...
listint_t *insert_nodeint_at_index(listint_t **head, unsigned int idx, int n)
{
	...
	if (new_node == NULL)
		return (NULL);
	/* accese new_node and initialize it as n */
	new_node->n = n;
	/* check if idx = 0 */
	if (idx == 0)
	{
		/* access the next and assign it as first node */
		new_node->next = *head;
		*head = new_node;
		return (new_node);
	}
